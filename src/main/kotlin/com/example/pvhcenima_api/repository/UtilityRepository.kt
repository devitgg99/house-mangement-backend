...
interface UtilityRepository : JpaRepository<Utility, UUID> {
    ...
    // Find all utilities for rooms owned by a house owner (Room → Floor → House → Owner)
    fun findAllByRoomFloorHouseOwnerUserId(ownerId: UUID): List<Utility>
    ...
    // Find utilities by house (Room → Floor → House)
    fun findAllByRoomFloorHouseHouseId(houseId: UUID): List<Utility>
    
    // Find utilities by house and month
    fun findAllByRoomFloorHouseHouseIdAndMonth(houseId: UUID, month: LocalDate): List<Utility>
